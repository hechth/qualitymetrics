    ...
    if (CV) {
        ...
        if (is.null(meta.samp.data$sampleType)) {
            ...
            if (!("pool" %in% levels(factor(meta.samp.data$sampleType)))) {
                ...
                if (
                    (!("sample" %in%
                        levels(factor(meta.samp.data$sampleType)))) &
                        (Compa)
                ) {
                    err.stock <- c(
                        err.stock,
                    ...
                    # Statement
                    tmp.ion <- data.frame(
                        CV.ind = rep(NA, nrow(ion.data)),
                        CV.samp = rep(NA, nrow(ion.data)),
                        CV.pool = rep(NA, nrow(ion.data)),
                        ion.data,
                        stringsAsFactors = FALSE
                    )
                    # CV samples
                    tmp.samp <- which(
                        colnames(tmp.ion) %in%
                            meta.samp.data[
                                which(meta.samp.data$sampleType == "sample"),
                                1
                            ]
                    )
                    tmp.ion$CV.samp <- apply(
                        tmp.ion[, tmp.samp],
                        1,
                        function(x) sd(x, na.rm = TRUE)
                    ) /
                        rowMeans(tmp.ion[, tmp.samp], na.rm = TRUE)
                    tmp.ion$CV.samp[which(
                        apply(
                            tmp.ion[, tmp.samp],
                            1,
                            function(x) sd(x, na.rm = TRUE)
                    ...
                    tmp.samp <- which(
                        colnames(tmp.ion) %in%
                            meta.samp.data[
                                which(meta.samp.data$sampleType == "pool"),
                                1
                            ]
                    )
                    tmp.ion$CV.pool <- apply(
                        tmp.ion[, tmp.samp],
                        1,
                        function(x) sd(x, na.rm = TRUE)
                    ) /
                        rowMeans(tmp.ion[, tmp.samp], na.rm = TRUE)
                    tmp.ion$CV.pool[which(
                        apply(
                            tmp.ion[, tmp.samp],
                            1,
                            function(x) sd(x, na.rm = TRUE)
                    ...
                    # CV indicator
                    if (Compa) {
                        tmp.ion$CV.ind <- ifelse(
                            (tmp.ion$CV.pool) / (tmp.ion$CV.samp) > seuil,
                            0,
                            1
                        )
                    } else {
                        tmp.ion$CV.ind <- ifelse(
                            (tmp.ion$CV.pool) > seuil,
                            0,
                            1
                        )
                    }
                    ...
                    if (Compa) {
                        tmp.ion <- tmp.ion[, c(4, 2, 3, 1, 1)]
                    } else {
                        tmp.ion <- tmp.ion[, c(4, 3, 1, 1)]
                    }
                    ...
                    meta.ion.data <- meta.ion.data[
                        order(meta.ion.data[, ncol(meta.ion.data)]),
                    ][, -ncol(meta.ion.data)]
...
qualityMetricsF <- function(
    ...
    allDigF <- function(string) {
        ## from the Hmisc package (all.digits)
        k <- length(string)
        result <- logical(k)
        for (i in 1:k) {
            st <- string[i]
    ...
    datPloF <- function() {
        ...
        thrVn <- c(pvalue = 0.001, poolCv = 0.3)

        ## Constants

        marLs <- list(
            dri = c(2.1, 2.6, 1.1, 1.1),
        ...
        axiPreF <- function(valVn, lenN) {
            ...
            if (lenN < length(valVn))
                stop(
                    "The length of in vector must be inferior to the length of the length parameter."
                )
            ...
            for (n in 1:length(preValVn))
                if (min(valVn) < preValVn[n] && preValVn[n] < max(valVn)) {
                    ...
                    preAtVn <- c(
                        preAtVn,
                        which(
                            abs(valVn - preValVn[n]) ==
                                min(abs(valVn - preValVn[n]))
                        )[1]
                    )
                }

            return(list(atVn = preAtVn, labVn = preLabVn))
        }

        colF <- function(vecVn)
            sapply(vecVn, function(outN) {
                if (outN < ploRgeVn[1]) return(palHeaVc[1]) else if (
                    outN > ploRgeVn[2]
                )
                    return(palHeaVc[256]) else
                    return(palHeaVc[round(
                        (outN - ploRgeVn[1]) / diff(ploRgeVn) * 256 + 1
                    )])
            })
        ...
        obsColF <- function(typVc) {
            ...
            if ("pool" %in% samTypVc) samColVc[grepl("pool", samTypVc)] <- "red"
            ...
            while (any(samColVc == "")) {
                ...
                if (palColI <= palColMaxI)
                    samColVc[typToColI] <- palVc[palColI] else
        ...
        par(font = 2, font.axis = 2, font.lab = 2, pch = 18)

        layout(
            matrix(
                c(1, 3, 4, 5, 5, 1, 7, 7, 7, 6, 2, 7, 7, 7, 6),
                byrow = TRUE,
                nrow = 3
            ),
        ...
        } else obsColVc <- rep("black", nrow(samDF))
        ...
        mtext(
            paste("t1 (", round(vRelVn[1] * 100), "%)", sep = ""),
            cex = 0.7,
            line = 2,
            side = 1
        )
        mtext(
            paste("t2 (", round(vRelVn[2] * 100), "%)", sep = ""),
            cex = 0.7,
            las = 0,
            line = 2,
            side = 2
        ...
        lines(
            sqrt(var(ropScoreMN[, 1]) * hotFisN) * cos(radVn),
        ...
        if ("sampleType" %in% colnames(samDF)) {
            ...
            text(
                rep(par("usr")[1], times = length(obsColVuc)),
                par("usr")[3] +
                    (0.97 -
                        length(obsColVuc) * 0.03 +
                        1:length(obsColVuc) * 0.03) *
                        diff(par("usr")[3:4]),
                col = obsColVuc,
                font = 2,
        ...
        plot(
            ...
            xlab = "",
            ylab = "",
            xlim = c(min(missZscoVn), max(missZscoVn) + 0.5)
        )
        mtext(
            "amount of missing values (z-score)",
            cex = 0.7,
            line = 2,
            side = 1
        )
        mtext("deciles (zscore)", cex = 0.7, las = 0, line = 2, side = 2)
        ...
        text(
            ...
            adj = 0,
            cex = 1.1,
            labels = paste0("min: ", signif(min(datMN, na.rm = TRUE), 2))
        )
        text(
            1,
            0.55,
            adj = 0,
            cex = 1.1,
            labels = paste0("median: ", signif(median(datMN, na.rm = TRUE), 2))
        )
        text(
            1,
            0.45,
            adj = 0,
            cex = 1.1,
            labels = paste0("mean: ", signif(mean(datMN, na.rm = TRUE), 2))
        )
        text(
            1,
            0.35,
            adj = 0,
            cex = 1.1,
            labels = paste0("max: ", signif(max(datMN, na.rm = TRUE), 2))
        )
        if (
            "sampleType" %in%
                colnames(samDF) &&
            ...
            text(
                ...
                labels = paste0(
                    ...
                    round(
                        sum(
                            varDF[!poolCvNanVl, "pool_CV", drop = FALSE] <
                                thrVn["poolCv"]
                        ) /
                            nrow(varDF) *
                            100
        ...
        if ("injectionOrder" %in% colnames(driSamDF)) {
            if ("batch" %in% colnames(driSamDF))
                ...
                ) else ordVi <- order(driSamDF[, "injectionOrder"])
        } else ordVi <- 1:nrow(driDatMN)
        ...
        mtext("injection order", cex = 0.7, line = 2, side = 1)

        mtext("Sum of intens. for all variables", cex = 0.7, line = 2, side = 2)
        ...
        par(mar = marLs[["msd"]])
        plot(
            apply(datMN, 2, function(y) mean(y, na.rm = TRUE)),
            apply(datMN, 2, function(y) sd(y, na.rm = TRUE)),
            pch = 18,
            xlab = "",
            ylab = ""
        )
        mtext("mean", cex = 0.7, line = 2, side = 1)
        mtext("sd", cex = 0.7, line = 2, side = 2)
        ...
        eval(parse(
            text = paste(
                "axis(at = axiPreF(c(ifelse(min(datMN, na.rm = TRUE) == -Inf, yes = 0, no = min(datMN, na.rm = TRUE)) , max(datMN, na.rm = TRUE)), 256)$atVn,
             font = 2,
             font.axis = 2,
        ...
        } else rowNamVc <- rownames(datMN)

        if (length(colnames(datMN)) == 0) {
            colNamVc <- rep("", times = ncol(datMN))
        } else colNamVc <- colnames(datMN)

        xlaVc <- paste(
            paste(rep("[", 2), c(1, nrow(imaMN)), rep("] ", 2), sep = ""),
            rep("\n", times = 2),
    ...
    zScoreF <- function(x) {
        ...
        if (sdxN < epsN) return(rep(0, length(x))) else
    ...
    if (
        "sampleType" %in% colnames(samDF) && "blank" %in% samDF[, "sampleType"]
    ) {
        cat("\nVariables: Blank mean, sd, and CV\n", sep = "")

        blkVl <- samDF[, "sampleType"] == "blank"

        if (sum(blkVl) == 1) varDF[, "blank_mean"] <- datMN[blkVl, ] else
            varDF[, "blank_mean"] <- apply(
                datMN[blkVl, , drop = FALSE],
                2,
                function(varVn) mean(varVn, na.rm = TRUE)
            )

        if (sum(blkVl) == 1) varDF[, "blank_sd"] <- rep(0, nrow(varDF)) else
            varDF[, "blank_sd"] <- apply(
                datMN[blkVl, , drop = FALSE],
                2,
    ...
    if (
        "sampleType" %in% colnames(samDF) && "sample" %in% samDF[, "sampleType"]
    ) {
        cat("\nVariables: Sample mean, sd, and CV\n", sep = "")

        samVl <- samDF[, "sampleType"] == "sample"

        if (sum(samVl) == 1) varDF[, "sample_mean"] <- datMN[samVl, ] else
            varDF[, "sample_mean"] <- apply(
                datMN[samVl, , drop = FALSE],
                2,
                function(varVn) mean(varVn, na.rm = TRUE)
            )

        if (sum(samVl) == 1) varDF[, "sample_sd"] <- rep(0, nrow(varDF)) else
            varDF[, "sample_sd"] <- apply(
                datMN[samVl, , drop = FALSE],
                2,
                function(varVn) sd(varVn, na.rm = TRUE)
            )
    ...
    if (all(c("blank_mean", "sample_mean") %in% colnames(varDF))) {
        ...
        varDF[, "blankMean_over_sampleMean"] <- varDF[, "blank_mean"] /
            varDF[, "sample_mean"]
    }
    ...
    if (
        "sampleType" %in% colnames(samDF) && "pool" %in% samDF[, "sampleType"]
    ) {
        cat("\nVariables: Pool mean, sd, and CV\n", sep = "")

        pooVl <- samDF[, "sampleType"] == "pool"

        if (sum(pooVl) == 1) varDF[, "pool_mean"] <- datMN[pooVl, ] else
            varDF[, "pool_mean"] <- apply(
                datMN[pooVl, , drop = FALSE],
                2,
                function(varVn) mean(varVn, na.rm = TRUE)
            )

        if (sum(pooVl) == 1) varDF[, "pool_sd"] <- rep(0, nrow(varDF)) else
            varDF[, "pool_sd"] <- apply(
                datMN[pooVl, , drop = FALSE],
                2,
                function(varVn) sd(varVn, na.rm = TRUE)
    ...
    if (all(c("pool_CV", "sample_CV") %in% colnames(varDF))) {
        ...
        varDF[, "poolCV_over_sampleCV"] <- varDF[, "pool_CV"] /
            varDF[, "sample_CV"]
    }
    ...
    if (
        "sampleType" %in%
            colnames(samDF) &&
            any(grepl("pool.+", samDF[, "sampleType"]))
    ) {
        ...
        if (sum(pooDilVl)) {
            ...
            varDF[, "poolDil_correl"] <- apply(
                datMN[pooVi, , drop = FALSE],
                2,
                function(varVn) cor(dilVn, varVn)
            )

            varDF[, "poolDil_pval"] <- apply(
                datMN[pooVi, , drop = FALSE],
                2,
    ...
    hotPvaVn <- apply(
        ...
        function(x)
            1 -
                pf(
                    1 / hotN * t(as.matrix(x)) %*% invCovScoMN %*% as.matrix(x),
                    2,
                    n - 2
                )
    )
    ...
    missZscoVn <- zScoreF(apply(datMN, 1, function(rowVn) {
        sum(is.na(rowVn))
    }))

    samDF[, "missing_pval"] <- sapply(
        missZscoVn,
        function(zscoN) 2 * (1 - pnorm(abs(zscoN)))
    )
    ...
    deciZscoMaxVn <- apply(
        deciZscoMN,
        1,
        function(rowVn) rowVn[which.max(abs(rowVn))]
    )

    samDF[, "decile_pval"] <- sapply(
        deciZscoMaxVn,
        function(zscoN) 2 * (1 - pnorm(abs(zscoN)))
    )
    ...
    } else dev.new(width = 11, height = 7)

    datPloF()

    if (!is.null(fig.pdfC)) dev.off()
    ...
    return(list(samDF = samDF, varDF = varDF))
} ## qualityMetricsF
